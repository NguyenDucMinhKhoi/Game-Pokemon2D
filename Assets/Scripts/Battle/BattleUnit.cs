...
public class BattleUnit : MonoBehaviour
{
    ...
    public IEnumerator PlayCaptureAnimation()
    {
        ...
        sequece.Append(image.DOFade(0, 0.5f));
        sequece.Join(transform.DOLocalMoveY(orginalPos.y + 50f, 0.5f));
        sequece.Join(transform.DOScale(new Vector3(0.3f, 0.3f, 1f), 0.5f));
        yield return sequece.WaitForCompletion();
    }
    
    public IEnumerator PlayBreakOutAnimation()
    {
        var sequece = DOTween.Sequence();
        sequece.Append(image.DOFade(1, 0.5f));
        sequece.Join(transform.DOLocalMoveY(orginalPos.y, 0.5f));
        sequece.Join(transform.DOScale(new Vector3(1f, 1f, 1f), 0.5f));
        yield return sequece.WaitForCompletion();
    }    
}
